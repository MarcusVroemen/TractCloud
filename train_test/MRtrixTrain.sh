...
k_global="50"   # global, randomly selected streamlines in the whole-brain
...
# Paths
local_global_rep_folder=k${k}_kg${k_global}_ds${k_ds_rate}_kp${k_point_level}_bs${batch_size}_nf${num_f_brain}_np${num_p_fiber}_epoch${epoch}_lr${lr}_THR${threshold} #_classweight${class_weighting}_decays${weight_decay}${decay_factor}_FE${fibersampling}_${atlas}_${threshold}_MASK
...
# input_path=/media/volume/HCP_diffusion_MV/TrainData_${input_data}_${encoding}_MNI
...
# # Paths
# local_global_rep_folder=10L_k${k}_kg${k_global}_ds${k_ds_rate}_kp${k_point_level}_bs${batch_size}_nf${num_f_brain}_np${num_p_fiber}_epoch${epoch}_lr${lr}_THR${threshold} #_classweight${class_weighting}_decays${weight_decay}${decay_factor}_FE${fibersampling}_${atlas}_${threshold}_MASK
# out_path=../ModelWeights/Data${input_data}_Rot${rot_ang_lst}Scale-${scale_ratio_range}Trans${trans_dis}AugTimes${aug_times}_Unrelated100HCP_${model_name}/${local_global_rep_folder}
# # input_path=/media/volume/HCP_diffusion_MV/TrainData_${input_data}_${encoding}_MNI
# input_path=/media/volume/MV_HCP/TrainData_${input_data}

# # Train/Validation/Test --recenter --include_org_data --use_tracts_testing --mark_endpoints
# python train.py --depth 10 --threshold ${threshold} --fibersampling ${fibersampling} --weight_decay ${weight_decay} --decay_factor ${decay_factor} --encoding ${encoding} --class_weighting ${class_weighting} --connectome --atlas ${atlas} --k_ds_rate ${k_ds_rate} --rot_ang_lst ${rot_ang_lst} --scale_ratio_range ${scale_ratio_range} --trans_dis ${trans_dis} --aug_times ${aug_times} --k ${k} --k_point_level ${k_point_level} --k_global ${k_global} --num_fiber_per_brain ${num_f_brain} --num_point_per_fiber ${num_p_fiber} --input_path ${input_path} --epoch ${epoch} --out_path_base ${out_path} --model_name $model_name --train_batch_size ${batch_size} --val_batch_size ${batch_size} --test_batch_size ${batch_size}  --lr ${lr}
# python test.py --depth 10 --threshold ${threshold} --fibersampling ${fibersampling} --encoding ${encoding} --connectome --atlas ${atlas} --out_path_base ${out_path} --aug_times ${test_aug_times} --input_path ${input_path}
